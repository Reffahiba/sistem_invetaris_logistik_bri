...
const EditBarangModal: React.FC<EditBarangModalProps> = ({
  ...
  const handleDrop = useCallback((e: React.DragEvent<HTMLDivElement>) => { e.preventDefault(); e.stopPropagation(); setIsDragging(false); handleFileChange(e.dataTransfer.files); }, [handleFileChange]);
  ...
  const handleEditSubmit = async (e: FormEvent) => {
    ...
    formData.append('id_kategori', editData.id_kategori ? String(editData.id_kategori) : '');
    ...
    try {
      await axios.post(`/api/admin/barang/${itemToEdit.id_barang}`, formData, {
  ...
  const previewImageSrc = uploadedFile
    ...
    : (typeof editData.gambar_barang === 'string' ? editData.gambar_barang : null); // Tampilkan URL lama jika tidak
      ...
      <div ref={modalRef} className="bg-white p-6 rounded-lg shadow-xl w-11/12 max-w-[500px] flex flex-col max-h-[90vh]">
        ...
        <div className="flex-grow overflow-y-auto pr-4 -mr-4">
          <form onSubmit={handleEditSubmit} className="grid gap-4">
            {/* Field Nama Barang */}
            <div>
              <label htmlFor="nama_barang_edit" className="text-sm font-medium text-gray-700 mb-1 block">Nama Barang <span className="text-red-500">*</span></label>
              <input type="text" id="nama_barang_edit" className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-0.5rem focus:ring-[#F46F23] focus:border-[#F46F23]" value={editData.nama_barang} onChange={(e) => setEditData({ ...editData, nama_barang: e.target.value })} required />
            </div>
            
            {/* Field Kategori */}
            <div>
              <label htmlFor="id_kategori_edit" className="text-sm font-medium text-gray-700 mb-1 block">Kategori <span className="text-red-500">*</span></label>
              <div className="relative">
                <select id="id_kategori_edit" className="w-full px-3 py-2 border border-gray-300 rounded-md appearance-none focus:outline-none focus:ring-0.5rem focus:ring-[#F46F23] focus:border-[#F46F23]" value={editData.id_kategori ?? ''} onChange={(e) => setEditData({ ...editData, id_kategori: e.target.value ? Number(e.target.value) : null })} required>
                  <option value="">-- Pilih Kategori --</option>
                  {kategoriOptions.map((k) => (<option key={k.id_kategori} value={k.id_kategori}>{k.nama_kategori}</option>))}
                </select>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <label htmlFor="stok_edit" className="text-sm font-medium text-gray-500 mb-1 block">
                    Stok
                    </label>
                    <input
                    type="number"
                    id="stok_edit"
                    className="w-full px-3 py-2 border border-gray-300 rounded-md disabled:bg-gray-100 disabled:cursor-not-allowed" // 2. Tambahkan class untuk styling saat disabled
                    value={editData.stok}
                    min="0"
                    ...
                    <div>
                        <label htmlFor="satuan_edit" className="text-sm font-medium text-gray-700 mb-1 block">
            ...
            {/* Area Upload Gambar */}
            <div>
              <label className="text-sm font-medium text-gray-700 mb-1 block">Ganti Gambar Barang</label>
              <div className={`flex flex-col items-center justify-center p-6 border-2 border-dashed rounded-lg bg-gray-50 text-gray-600 transition-all cursor-pointer ${isDragging ? 'border-secondary' : 'border-gray-300'}`} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop} onClick={handleBrowseClick}>
                <input type="file" ref={fileInputRef} onChange={(e) => handleFileChange(e.target.files)} className="hidden" accept="image/jpeg,image/jpg,image/png"/>
                {previewImageSrc ? (
                  <>
                    <FileImage className="h-10 w-10 mb-2 text-primary" />
                    <p className="text-sm font-medium text-primary mb-1">Pratinjau Gambar:</p>
                    <img src={previewImageSrc} alt="Preview" className="max-w-full h-24 object-contain rounded-md shadow-sm mt-2" />
                    <p className="text-xs text-gray-500 mt-2">Klik atau seret file baru untuk mengganti</p>
                  </>
                ) : (
                  <>
                    <UploadCloud className="h-10 w-10 mb-2" />
                    <p className="text-sm font-medium">Seret dan lepas gambar di sini, atau <span className="text-primary font-semibold">klik untuk memilih</span></p>
            ...
            <div className="flex justify-end space-x-3 pt-4 border-t border-gray-200 mt-4">
              <button type="button" onClick={onClose} className="px-4 py-2 bg-gray-300 rounded-lg" disabled={isLoading}>Batal</button>
              <button type="submit" className="px-4 py-2 bg-primary text-white rounded-lg" disabled={isLoading}>
                {isLoading ? <Loader2 className="animate-spin" /> : 'Simpan Perubahan'} {/* <-- Teks diubah */}
              </button>
            </div>
          </form>
        </div>
