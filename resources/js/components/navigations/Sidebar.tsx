    ...
    return (
      <li>
        <button
          ...
          className={`w-full flex items-center justify-between p-2 mb-3 rounded-lg text-sm transition-colors duration-300 ease-in-out ${isActive ? "text-blue-600 font-semibold" : "text-gray-700 hover:bg-gray-200"}`}
        >
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      ...
      <nav className="flex-1">
        <ul>
          {navItems.map((item) => {
            const isSubItemActive = item.subItems?.some(sub => sub.href === currentPath) || false;
