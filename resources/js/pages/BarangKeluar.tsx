...
const SortArrow = ({ order }: { order: 'asc' | 'desc' }) => (
    <div className="inline-flex flex-col items-center justify-center ml-1"><ChevronUp className={`h-3 w-3 ${order === 'asc' ? 'text-gray-800' : 'text-gray-400'}`} /><ChevronDown className={`h-3 w-3 -mt-1 ${order === 'desc' ? 'text-gray-800' : 'text-gray-400'}`} /></div>
);
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pb-6 border-b border-gray-200 mb-6">
                ...
                <div className="relative w-full sm:w-48">
                    ...
                    >
                        {[10, 25, 50, 100].map(n => <option key={n} value={n}>{n} entri / halaman</option>)}
                    </select>
                    <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-3 text-gray-500">
                        <ChevronDown size={16} className="transform transition-transform duration-200 group-focus-within:rotate-180" />
                    </div>
                </div>
            </div>
            ...
            <table className="min-w-full">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">ID Permintaan</th>
                  <th onClick={() => handleSort('tanggal_minta')} className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase cursor-pointer">
                    <div className="flex items-center">Tanggal {sortBy === 'tanggal_minta' && <SortArrow order={sortOrder} />}</div>
                  </th>
                  <th onClick={() => handleSort('peminta')} className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase cursor-pointer">
                    <div className="flex items-center">Peminta {sortBy === 'peminta' && <SortArrow order={sortOrder} />}</div>
                  </th>
                  <th onClick={() => handleSort('barang')} className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase cursor-pointer">
                    <div className="flex items-center">Barang {sortBy === 'barang' && <SortArrow order={sortOrder} />}</div>
                  </th>
              ...
              <tbody className="bg-white divide-y">
                 {isLoading ? (
                  ...
                  data.map((trans, index) => (
                    <tr key={trans.id_detail} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 font-medium text-gray-700">{trans.permintaan.user.nama}</td>
                      <td className="px-6 py-4 font-medium text-gray-700">{trans.barang.nama_barang}</td>
